    ...
    <div class="px-4 pt-5 pb-10 w-full max-w-[430px] mx-auto">
        ...
        <div class="flex items-center mb-5">
            ...
            <form class="flex-1 mr-2">
                {{-- Pertahankan filter tanggal saat search --}}
                @if($startDate)
        ...
        <form method="GET" class="mb-4">
            {{-- PERBAIKAN 1: Inject Tanggal & Search agar tidak hilang saat klik Type --}}
            @if($startDate)
                <input type="hidden" name="start_date" value="{{ $startDate }}">
                <input type="hidden" name="start_time" value="{{ $startTime }}">
                <input type="hidden" name="end_date" value="{{ $endDate }}">
            ...
            @foreach ($items as $item)
                @php
                    ...
                    $image = ($item->image_asset && file_exists(public_path('uploads/assetmasters/' . $item->image_asset)))
                        ...
                        : asset('images/no_images.png'); 
                @endphp

                {{-- PERBAIKAN 2: Kirim $availableCount ke fungsi openDetail --}}
                <div onclick="openDetail('{{ $item->id_master }}', {{ $availableCount }})"
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="date" name="end_date" required value="{{ $endDate }}"
                                class="flex-1 h-10 border border-gray-300 rounded-lg px-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#F26E21]">
                            <input type="time" name="end_time" required value="{{ $endTime }}"
                                class="flex-1 h-10 border border-gray-300 rounded-lg px-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#F26E21]">
                        </div>
                    </div>
                </div>
