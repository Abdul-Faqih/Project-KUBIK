...
@section('content'){{-- WRAPPER LAYAR --}}
    <div class="min-h-screen flex justify-center items-start pt-5 pb-10 bg-white">

        <div class="w-full max-w-[360px]">

            {{-- === HEADER === --}}
            <div class="flex items-center gap-3 mb-8">
                {{-- UBAH DISINI: Ganti href jadi onclick --}}
            ...
            {{-- === FORM === --}}
            {{-- UBAH DISINI: Tambahkan id="registerForm" --}}
            <form action="{{ route('user.register.submit') }}" method="POST" id="registerForm">
                ...
                {{-- EMAIL --}}
                <div class="mb-4 space-y-1">
                    <input type="email" name="email" id="emailInput" class="input-auth" placeholder="Email Address (@pradita.ac.id)" value="{{ old('email') }}" autocomplete="off">
                    <p class="text-[11px] text-gray-400 ml-2 mt-1 italic">*Must use @student.pradita.ac.id or @pradita.ac.id</p>
                </div>
                ...
                {{-- BUTTON (UBAH DISINI: type="button" onclick="validateAndOpenModal()") --}}
