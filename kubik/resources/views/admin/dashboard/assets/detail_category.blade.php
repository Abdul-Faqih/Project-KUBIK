...
@section('content')
    <div class="-mt-5">
        <p class="text-base text-[#2A2A2A] mb-2">
            <a href="{{ route('admin.dashboard.categories') }}" class="hover:underline text-[#F26E21]">Categories</a> >
        ...
        <div class="bg-[#FBFBFB] rounded-2xl shadow p-8 relative">
            <form id="editForm" method="POST"
                action="{{ route('admin.dashboard.categories.update', $category->id_category) }}">
                @csrf
                <div class="flex items-start justify-between">
                    <div>
                        <h2 class="text-[#F26E21] font-bold text-xl mb-1">{{ $category->id_category }}</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-4 mt-3">
                    <div>
                        <label class="block text-[#2A2A2A] text-base font-base mb-1">ID Category</label>
                        <input type="text" value="{{ $category->id_category }}"
                    ...
                    <div>
                        ...
                        <input id="nameField" type="text" name="name" value="{{ $category->name }}"
    ...
    <div id="deleteModal"
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 w-[380px]">
            ...
            <p class="text-base text-[#2A2A2A] mb-7">
                ...
                <span class="font-semibold text-[#F26E21]">{{ $category->id_category }}</span>?<br>
                This action cannot be undone.
            </p>

            <form id="deleteForm" method="POST"
                action="{{ route('admin.dashboard.categories.delete', $category->id_category) }}">
                @csrf
                @method('DELETE')
